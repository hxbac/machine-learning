...
num_epochs = 40
#create a network model without adding L2 weight regularization
...
#create a network model with adding L2 weight regularization
model = models.Sequential()
model.add(layers.Dense(256, kernel_regularizer=regularizers.l2(0.001), activation='relu',input_shape =(28*28,)))
model.add(layers.Dense(10, activation='softmax'))
model.compile(optimizer='rmsprop', loss='categorical_crossentropy', metrics=['accuracy'])
...
plt.plot(history1.history['val_loss'],'-b+',label = 'The model without adding L2 weight regularization')
plt.plot(history2.history['val_loss'],'-rx',label = 'The model with adding L2 weight regularization')
plt.ylabel('Loss')
plt.xlabel('Epochs')
